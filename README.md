...
- `04_eval.py`：读评测集 JSON（`--queries`，含 `queries[{zh,targets}]`），对配置组合（`siglip2+tr` / `siglip2+zh2en` / `cnclip+raw`）分别建索引→逐查询→输出视频级 hit@1/5/10 + MRR，报告写 `--out`（默认 `eval_local/report.json`）。评测数据含公司素材名，放 `eval_local/`（已 `.gitignore`，不入库）。
